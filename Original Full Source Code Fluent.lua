...
Components.Window = (function()
	...
	return function(Config)
		...
		function Window:Minimize()
			...
			local scaleGoal = self.Minimized and 0.9 or 1
			local transparencyGoal = self.Minimized and 0.5 or 0
			
			Creator.SpringMotor(scaleGoal, self.Root, "Size", true)
			Creator.SpringMotor(transparencyGoal, self.Root, "BackgroundTransparency", true)
			
			-- Icon rotation animation
			local rotateMotor = Flipper.SingleMotor.new(0)
			rotateMotor:onStep(function(value)
				self.TitleBar.MinButton.Frame.Icon.Rotation = value
			end)
			rotateMotor:setGoal(Flipper.Spring.new(self.Minimized and 180 or 0, {frequency = 6}))
		end
